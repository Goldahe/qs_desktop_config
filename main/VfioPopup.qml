...
LifecyclePopup {
    ...
    function prepareClose() {
        closePending = true
        // Assignment actions are never cancelled or orphaned by dismissal.
        if (actionRunning) return false
        pollingStopped = true
        refreshTimer.stop()
        actionFinishTimer.stop()
        if (statusProcess.running) { statusProcess.signal(15); return false }
        return true
    }
    ...
    readonly property bool actionRunning: actionInFlight
    readonly property bool startEnabled: stateKnown && !vfioMode && !actionRunning && !closePending
    readonly property bool stopEnabled: stateKnown && vfioMode && !actionRunning && !closePending
    ...
    function refresh() {
        if (!pollingStopped && !statusProcess.running)
            statusProcess.running = true
    }

    function togglePopup() {
        if (visible) lifecycleOwner.close()
        else visible = true
    }

    function runAction(name, command) {
        if (actionRunning || closePending)
            return
        actionInFlight = true
        actionName = name
        actionStdout = ""
        ...
        // A guaranteed launcher converts a missing target into an ordinary
        // exit status, since QS 0.3.1 has no reliable failed-start QML signal.
        actionProcess.command = ["/usr/bin/env", "--"].concat(command)
    ...
    function finishAction(exitCode) {
        ...
        if (closePending) lifecycleOwner.close()
        else refresh()
    }
    ...
    Process {
        id: statusProcess
        running: false
        command: ["python", Qt.resolvedUrl("poll-session.py").toString().replace("file://", ""), Qt.resolvedUrl("vfio-status.sh").toString().replace("file://", "")]
        onExited: if (vfioPopup.pollingStopped) lifecycleOwner.close()
        stdout: StdioCollector {
            onStreamFinished: if (!vfioPopup.pollingStopped) vfioPopup.updateStatus(this.text)
        }
    }
    ...
    Timer {
        id: refreshTimer
        interval: 2000
        running: vfioPopup.visible && !vfioPopup.pollingStopped
        repeat: true
