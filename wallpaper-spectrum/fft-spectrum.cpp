// stdin: interleaved float32le stereo, 48000 Hz.
// stdout: exactly 640 little-endian float32 values (2560 bytes) per hop.
...
public:
    ...
    bool frame(const float *input) {
        ...
        return std::fwrite(previous.data(), sizeof(float), BINS, stdout) == BINS &&
               std::fflush(stdout) == 0;
    }
};
}
