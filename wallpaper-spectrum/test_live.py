...
try:
    ...
    with (OUT/'quickshell.log').open('w') as log:
        ...
        assert before['wallpaperColorEffectEnabled'] is True
        assert before['wallpaperColorEffectAmount'] == 0
        ...
        assert active['wallpaperColorEffectAmount'] == 1
        ...
        time.sleep(2)
        held=ipc_json('status')
        assert held['peak']<0.001 and held['wallpaperColorEffectAmount'] == 1, held
        screenshot('idle-held.png')
        time.sleep(16)
        end=ipc_json('status')
        assert end['peak']<0.001 and end['wallpaperColorEffectAmount'] == 0, end
        ...
        assert bbox and bbox[1] < base.height()//2, ('Wallpaper color effect did not reach full screen',bbox)
        held_img=QImage(str(OUT/'idle-held.png'))
        assert difference_bbox(base,held_img) is not None, 'Idle hold returned to normal too early'
