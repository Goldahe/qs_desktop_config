#!/usr/bin/env python3
"""Dependency-free stdin/binary contract tests. Run: python3 test_fft.py."""
import array
import math
import os
import struct
...
BINARY = Path(os.environ.get('SPECTRUM_ANALYZER', ROOT / 'fft-spectrum'))
RATE = 48000
HOP = 2000
BINS = 640
FRAME_BYTES = BINS * 4
...
class SpectrumTests(unittest.TestCase):
    def analyze(self, data):
        ...
        self.assertEqual(len(result.stdout) % FRAME_BYTES, 0,
                         'analyzer emitted a partial binary frame')
        frames = [list(struct.unpack('<640f', result.stdout[offset:offset + FRAME_BYTES]))
    ...
    def test_first_hop_is_flushed_without_eof(self):
        ...
        with subprocess.Popen([str(BINARY)], stdin=subprocess.PIPE, stdout=subprocess.PIPE) as process:
            try:
                ...
                self.assertEqual(len(struct.unpack('<640f', frame)), BINS)
            finally:
                process.stdin.close()
