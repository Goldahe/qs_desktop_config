...
public:
    ...
    Q_INVOKABLE int binForStripe(int stripe) const {
        if (stripe < 0 || stripe >= 1280) return -1;
        return stripe < 640 ? stripe : 1279 - stripe;
    }
    Q_INVOKABLE int physicalXForStripe(int stripe, int physicalWidth) const {
        if (stripe < 0 || stripe >= 1280 || physicalWidth < 1280) return -1;
        return (stripe * physicalWidth + 1279) / 1280;
    }
...
protected:
    ...
    QSGNode *updatePaintNode(QSGNode *oldNode, UpdatePaintNodeData *) override {
        constexpr int bins = 640;
        constexpr int stripes = bins * 2;
        ...
        if (!node) {
            auto *geometry = new QSGGeometry(
                ...
                stripes * verticesPerStripe,
                stripes * indicesPerStripe,
        ...
        for (int stripe = 0; stripe < stripes; ++stripe) {
            ...
            const int bin = binForStripe(stripe);
