...
PanelWindow {
    ...
    readonly property string screenName: outputScreen.name
    readonly property string resolvedWallpaperSource: {
        if (screenName in Theme.perScreenWallpapers && Theme.perScreenWallpapers[screenName] !== "")
            return surface.expandHome(Theme.perScreenWallpapers[screenName])
        return surface.expandHome(Theme.wallpaperSource)
    }
    readonly property string normalizedSource: {
        if (resolvedWallpaperSource.startsWith("file://"))
            return resolvedWallpaperSource
        return "file://" + resolvedWallpaperSource
    }
